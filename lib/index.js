...
exports = module.exports = function (schema) {
  ...
  return function (ctx, next)  {
    ...
    ['headers', 'body', 'query', 'params', 'cookies'].forEach(function (key) {
      ...
      var entireContext = options.contextRequest ? ctx.request : null;
      if (schema[key]) validate(errors, ctx.request[key], schema[key], key, allowUnknown, entireContext);
    });

    if (errors && errors.length === 0) return next();

    ctx.body = new ValidationError(errors, options);
  };
};
